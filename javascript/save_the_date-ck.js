...
This file calculates the time to the wedding and also calls a AJAX file, Checkes the form validation and calls php. 
**/
var save_the_date = {
    defaults: {
        weddingDate: "",
    ...
    rsvp: function() {
        ...
        if (!Modernizr.input.placeholder) {
            var t = ["#name", "#email", "textarea", "#number"];
            $(t).each(function(e, t) {
        ...
        $("#rsvp form").submit(function() {
            var t = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/,
                n = $("#email").val(),
                r = $("#name").val(),
                i = $("#check-sangeet").is(":checked"),
                j = $("#check-wedding").is(":checked"),
                k = $("#check-pooja").is(":checked"),
                l = $("#check-reception").is(":checked"),
                s = $("#number").val(),
                o = !1;
            $(".error").removeClass("error");
            if (i !== true && j !== true && k !== true && l !== true) {
                document.getElementById("oii").innerHTML = "Kindly select atleast one event.";
                o = !0
            }
            if (s === "" || s === $("#number").attr("placeholder")) {
